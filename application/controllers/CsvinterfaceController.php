...
class CsvinterfaceController extends Zend_Controller_Action {
	...
	public function exportAction() {
		...
		if ($exportType == "all") {
			...
			$sql = "SELECT * INTO OUTFILE '" . $fileName . "' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\n' FROM participant, size, event, division WHERE participant.event=event.eid and participant.size=size.sid and participant.division=division.did and bibnumber >= " . $startBib . " and bibnumber <= " . $endBib . " ORDER BY bibnumber;";
			$db -> query($sql);
		}
